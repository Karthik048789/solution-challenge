  ...
  return (
    <section className="space-y-6">
      <div className="rounded-2xl glass p-6 shadow-sm">
        <h1 className="font-display text-3xl font-semibold text-slate-900 dark:text-white">Upload Dataset</h1>
        ...
        <div
          ...
          className={`mt-6 rounded-2xl border-2 border-dashed p-8 text-center transition ${
            isDragging
              ...
              : "border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 hover:border-teal-400 dark:hover:border-teal-500"
          }`}
        >
          <p className="text-sm text-slate-700 dark:text-slate-300">Drag and drop your CSV file here</p>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-500">or</p>
          <label className="mt-3 inline-flex cursor-pointer rounded-lg bg-slate-900 dark:bg-slate-100 px-4 py-2 text-sm font-semibold text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-200">
            Browse Files
          ...
          {file && (
            <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">
              Selected: <span className="font-medium text-slate-900 dark:text-white">{file.name}</span>
            </p>
          )}
        </div>
      ...
      {uploadResult && (
        <div className="rounded-2xl glass p-6 shadow-sm">
          <h2 className="font-display text-xl font-semibold text-slate-900 dark:text-white">Preview (First 10 Rows)</h2>
            ...
            <table className="min-w-full border-collapse text-sm">
              <thead>
                <tr>
                  {previewColumns.map((column) => (
                    <th
                      key={column}
                      className="border-b border-slate-200 dark:border-slate-800 px-3 py-2 text-left font-semibold text-slate-700 dark:text-slate-300"
                    >
                      {prettyHeader(column)}
                    </th>
                  ))}
              ...
              <tbody>
                {uploadResult.preview_rows.map((row, index) => (
                  <tr key={index} className="odd:bg-slate-50 dark:odd:bg-slate-800/30">
                    {previewColumns.map((column) => (
                      <td key={`${index}-${column}`} className="px-3 py-2 text-slate-600 dark:text-slate-400">
                        {String(row[column] ?? "")}
                      </td>
                    ))}
                  </tr>
