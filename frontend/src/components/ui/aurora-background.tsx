...
const AuroraBackground = ({ children, className = '' }: AuroraBackgroundProps) => {
  return (
    <div className={`relative min-h-screen overflow-hidden bg-slate-50 dark:bg-black w-full ${className}`}>
      {/* Aurora Background */}
      <div className="absolute inset-0 pointer-events-none">
        {/* Base aurora layer */}
        <div className="absolute inset-0 opacity-50 dark:opacity-70">
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-200/50 via-purple-200/40 to-indigo-200/50 dark:from-blue-900/40 dark:via-purple-900/30 dark:to-indigo-900/40"></div>
        </div>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-100/60 via-transparent to-white/30 dark:from-black/20 dark:via-transparent dark:to-black/10"></div>
      </div>
